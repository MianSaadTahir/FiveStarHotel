    ...
    public partial class AddRt : Form
    {
        ...
        private void AddRtBtn_Click(object sender, EventArgs e)
        {
               ...
               RtDb.IsDuplicateRt(username.Text, ConnectionString))
            {
                ...
                Dwellers Dweller = new Dwellers(username.Text, password.Text, int.Parse(age.Text), Cnic.Text, "rt", gender.Text.ToLower(), HostelDb.GetHostelByName(RtHostel.Text, ConnectionString));
                HostelDb.GetHostelByName(RtHostel.Text, ConnectionString).SetRt(Rt);
                RtDb.InsertRt(Rt, DwellerDB.GetDwellerId(username.Text, ConnectionString), ConnectionString);
                DwellerDB.AddDwellers(Dweller, ConnectionString);
